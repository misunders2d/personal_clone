...
def execute_cypher_query(query: str, params: dict) -> list:
    """
    ...
    if not URI or not AUTH[0] or not AUTH[1]:
        raise ValueError("Neo4j credentials (NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD) are not set in the environment.")
    ...
    with GraphDatabase.driver(URI, auth=AUTH) as driver:
        ...
        print(f"Query `{summary.query}` returned {len(records)} records in {summary.result_available_after} ms.")
